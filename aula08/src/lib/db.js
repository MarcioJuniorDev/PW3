...
export function updateItem(id, update)
{
    const items = loadItems().map(
        // carrega todos os itens cadastrados e aplica sobre os mesmos uma função (no formato lambda) que atualiza apenas os objetos com o id igual ao fornecido
        i => {
            // "i" é a variável que guarda cada um dos elementos da lista. Por exemplo: se loadItems() retornar [1, 10, 20], então i assumirá cada um desses valores 
            if (i.id === id)
            {
                return {...i, ...update}
            }
